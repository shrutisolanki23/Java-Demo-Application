node{
      ...
      stage('Publish Docker Image'){
         withCredentials([string(credentialsId: 'dockerpwdshruti', variable: 'dockerPWD')]) { 
               sh "docker login -u shrutisolanki23 -p ${dockerPWD}"
         }
        sh 'docker push shrutisolanki23/javademoapp_$JOB_NAME:$BUILD_NUMBER'
      ...
      stage('Deploy'){
         ...
         withCredentials([string(credentialsId: 'k8pwdshruti', variable: 'k8pwd')]) {
